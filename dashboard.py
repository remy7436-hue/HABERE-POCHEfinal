...
if df.empty:
    ...
    delta_temp = (
        round(derniere_mesure['temp_c'] - row_1h['temp_c'], 1)
        if row_1h is not None and row_1h['diff_1h'] <= timedelta(minutes=20)
        else 0.0
    )
    delta_hum = (
        int(derniere_mesure['humidity'] - row_1h['humidity'])
        if row_1h is not None and row_1h['diff_1h'] <= timedelta(minutes=20)
        else 0
    )
    delta_press = (
        round(derniere_mesure['pressure'] - row_1h['pressure'], 1)
        if row_1h is not None and row_1h['diff_1h'] <= timedelta(minutes=20)
        else 0.0
    )
    ...
    delta_hum_3h = (
        int(derniere_mesure['humidity'] - row_3h['humidity'])
        if row_3h is not None and row_3h['diff_3h'] <= timedelta(minutes=45)
        else 0
    )
